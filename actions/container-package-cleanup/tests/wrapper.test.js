...
describe("OctokitWrapper", () => {
  ...
  describe("listPackages", () => {
    it("calls listPackagesForOrganization when type=true", async () => {
      wrapper.listPackagesForOrganization = jest
        .fn()
        .mockResolvedValue(["pkg1"]);
      const result = await wrapper.listPackages("org", "container", true);
      expect(wrapper.listPackagesForOrganization).toHaveBeenCalledWith(
        "org",
        "container",
      );
    ...
    it("calls listPackagesForUser when type=false", async () => {
      ...
      expect(wrapper.listPackagesForUser).toHaveBeenCalledWith(
        "user",
        "container",
      );
  ...
  describe("listVersionsForPackage", () => {
    it("calls getPackageVersionsForOrganization when type=true", async () => {
      wrapper.getPackageVersionsForOrganization = jest
        .fn()
        .mockResolvedValue(["ver1"]);
      const result = await wrapper.listVersionsForPackage(
        "org",
        "container",
        "pkg",
        true,
      );
      expect(wrapper.getPackageVersionsForOrganization).toHaveBeenCalledWith(
        "org",
        "container",
        "pkg",
      );
    ...
    it("calls getPackageVersionsForUser when type=false", async () => {
      ...
      expect(wrapper.getPackageVersionsForUser).toHaveBeenCalledWith(
        "user",
        "container",
        "pkg",
      );
  ...
  describe("getManifestDigests", () => {
    it("returns digests from docker manifest inspect", async () => {
      const fakeStdout = JSON.stringify({
        manifests: [{ digest: "sha1" }, { digest: "sha2" }],
      });
      ...
      expect(mockExecPromise).toHaveBeenCalledWith(
        "docker manifest inspect ghcr.io/owner/pkg:tag",
      );
