 ...
 */
async function deleteSinglePackageVersion({
  wrapper,
  owner,
  packageType,
  packageName,
  ...
  log.dim(
    `Deleting ${owner}/${packageName} (${packageType}) - id: ${versionId}, (${detail})`,
  );
  ...
  log.lightSuccess(
    `✓ Deleted ${owner}/${packageName} (${packageType}) - id: ${versionId}, (${detail})`,
  );
}
...
async function deletePackageVersion(
  ...
  const totalDeletedVersions = filtered.reduce(
  ...
  log.info(
    `Starting deletion of package versions for owner: ${normalizedOwner}`,
  );
  ...
  for (const { package: pkg, versions } of filtered) {
    ...
    log.debug(
      `Preparing to delete ${versions.length} versions of ${normalizedOwner}/${normalizedPackageName} (${packageType})`,
      _MODULE,
    );
    log.dryrun(
      `[DRY-RUN] ${normalizedOwner}/${normalizedPackageName} (${packageType}) - ${versions.length} versions will NOT be deleted (dry-run mode)`,
    );
    ...
    for (let i = 0; i < versions.length; i += batchSize) {
      ...
      const promises = batch.map(async (version) => {
        ...
        if (dryRun) {
          log.dryrun(
            `[DRY-RUN] ${normalizedOwner}/${normalizedPackageName} (${packageType}) - version id: ${version.id} (${detail}) will NOT be deleted (dry-run mode)`,
          );
        ...
        try {
          ...
          if (isSkippableError(error)) {
            log.warn(
              `Skipped ${normalizedPackageName} v:${version.id} - ${error.message}`,
            );
            resultStatus.push({
              packageName: normalizedPackageName,
              versionId: version.id,
          ...
          log.error(
            `Failed ${normalizedPackageName} v:${version.id} - ${error.message}`,
          );
          resultStatus.push({
            packageName: normalizedPackageName,
            versionId: version.id,
      ...
      const newErrors = results.filter(
        (r) => !r.success && !r.skipped && !r.critical,
      ).length;
