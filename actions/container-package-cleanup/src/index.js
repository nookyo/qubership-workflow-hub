...
async function run() {
  ...
  for (const {
    ...
    const originalPackageData = packagesWithVersions.find(
      (p) => p.package.name === pkg.name,
    );
    const totalVersionCount = originalPackageData
      ? originalPackageData.versions.length
      : 0;
    ...
    if (
      totalVersionCount > 0 &&
      totalVersionCount === versionsToDelete.length
    ) {
      core.warning(
        `All versions of package ${pkg.name} are marked for deletion. Keeping the most recent tagged version and its layers.`,
      );
      ...
      const latestTaggedVersion = [...allVersions]
        .reverse()
        .find((v) => v.metadata?.container?.tags?.length > 0);

      if (latestTaggedVersion) {
        const tag = latestTaggedVersion.metadata.container.tags[0];
        core.info(
          `Latest tagged version is ${latestTaggedVersion.id} with tag ${tag}. Finding its layers.`,
        );
        ...
        try {
          ...
          const layersToKeep = allVersions.filter((v) =>
            digests.includes(v.name),
          );
          const idsToKeep = new Set([
            latestTaggedVersion.id,
            ...layersToKeep.map((l) => l.id),
          ]);

          core.info(
            `Keeping version ${latestTaggedVersion.id} and ${layersToKeep.length} associated layers.`,
          );
          versionsToDelete = versionsToDelete.filter(
            (v) => !idsToKeep.has(v.id),
          );
        } catch (error) {
          core.warning(
            `Could not get manifest for ${pkg.name}:${tag}. Will only keep the main tagged version. Error: ${error.message}`,
          );
          versionsToDelete = versionsToDelete.filter(
            (v) => v.id !== latestTaggedVersion.id,
          );
        }
      } else {
        core.warning(
          `No tagged versions found for package ${pkg.name}. Keeping the most recent version.`,
        );
    ...
    for (const version of versionsToDelete) {
      ...
      try {
        ...
        if (
          ...
          core.warning(
            `Skipping version ${version.id} (name: ${version.name}) for package ${pkg.name}: it is the last tagged version and cannot be deleted.`,
          );
        } else if (
          error.status === 400 &&
          error.message.includes("more than 5000 downloads")
        ) {
          core.warning(
            `Skipping version ${version.id} (name: ${version.name}) for package ${pkg.name}: it has more than 5000 downloads and cannot be deleted.`,
          );
        } else {
          throw error;
        }
