...
async function run() {
  ...
  const packages = await wrapper.listPackages(
    owner,
    package_type,
    isOrganization,
  );
  ...
  log.notice(
    `Total packages found: ${packages.length}, packages filtered by repo '${repo}': ${filteredPackages.length}`,
  );
  log.endGroup();
  ...
  const packagesWithVersions = await Promise.all(
    filteredPackages.map(async (pkg) => {
      ...
      log.info(
        `Found ${versionsForPkg.length} versions for package: ${pkg.name}`,
      );
  ...
  log.endGroup();
  log.notice(
    `Total packages to process: ${filteredPackages.length}, total versions found: ${totalPackagesVersions}`,
  );
  log.endGroup();
  ...
  try {
    if (filteredPackagesWithVersionsForDelete.length > 0) {
      deleteStatus = await deletePackageVersion(
        filteredPackagesWithVersionsForDelete,
        {
          wrapper,
          owner,
  ...
  deleteStatus.some((r) => r.success === false)
    ? core.setFailed(
        "❗️ Action completed with errors. Please check the logs and the report above.",
      )
...
async function showReport(context, type = "container") {
  const report =
    type === "container" ? new ContainerReport() : new MavenReport();
