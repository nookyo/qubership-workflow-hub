...
async function run() {
  ...
  for (const {
    ...
    if (
      ...
      core.warning(
        `All versions of package ${pkg.name} are marked for deletion. Keeping the version tagged 'latest' and its layers.`,
      );

      const allVersions = originalPackageData.versions;
      // Find the version with the 'latest' tag.
      const latestTaggedVersion = allVersions.find((v) =>
      ...
      if (latestTaggedVersion) {
        const tag = "latest";
        core.info(
          `Version ${latestTaggedVersion.id} is tagged 'latest'. Finding its layers.`,
        );
        ...
        core.warning(
          `No version with 'latest' tag found for package ${pkg.name}. Keeping the most recent version overall.`,
        );
