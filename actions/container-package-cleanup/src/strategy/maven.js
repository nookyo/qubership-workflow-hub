...
class MavenStrategy {
  ...
  async execute({ filteredPackages, thresholdDate }) {
    let filteredPackagesWithVersionsForDelete = packagesWithVersions;

    core.warning(
      `🔹Executing MavenStrategy with thresholdDate: ${thresholdDate}`,
    );
    core.warning(
      `🔹Filtered Packages: ${JSON.stringify(filteredPackages, null, 2)}`,
    );
