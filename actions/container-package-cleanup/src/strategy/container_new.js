...
class ContainerStrategy extends AbstractPackageStrategy {
  ...
  async execute({
    ...
    for (const pkg of packages) {
      ...
      if (debug) {
        core.info(
          `Package ${pkg.name}: Tagged to delete: ${taggedToDelete.map((v) => v.name).join(", ")}`,
        );
      }
      ...
      if (debug) {
        core.info(
          `Package ${pkg.name}: Orphan layers to delete: ${orphanLayers.map((v) => v.name).join(", ")}`,
        );
      }
      ...
      if (debug && danglingLayers.length) {
        core.info(
          `Package ${pkg.name}: Dangling layers: ${danglingLayers.map((v) => v.name).join(", ")}`,
        );
      }
