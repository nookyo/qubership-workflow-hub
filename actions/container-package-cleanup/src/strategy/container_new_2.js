...
class ContainerStrategy extends AbstractPackageStrategy {
  ...
  async execute({
    ...
    for (const { package: pkg, versions } of packagesWithVersions) {
        ...
        included.length > 0
          ? withoutExclude.filter((v) =>
              v.metadata.container.tags.some((tag) =>
                included.some((pat) => this.wildcardMatcher.match(tag, pat)),
              ),
            )
      ...
      debug &&
        core.debug(
          `Package ${pkg.name}: taggedToDelete = [${taggedToDelete.map((v) => v.name)}]`,
        );
          ...
          try {
            ...
            if (Array.isArray(manifest.layers)) {
              manifest.layers.forEach(
                (layer) => layer.digest && digs.add(layer.digest),
              );
            }
          } catch (e) {
            debug &&
      ...
      debug &&
        core.debug(
          `Package ${pkg.name}: layersToDelete = [${layersToDelete.map((v) => v.name)}]`,
        );
