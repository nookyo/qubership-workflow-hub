...
class ContainerStrategy extends AbstractPackageStrategy {
  ...
  execute({
    ...
    for (let { package: pkg, versions } of packagesWithVersions) {
      debug &&
        core.info(
          `Processing versions for package: ${JSON.stringify(versions, null, 2)}`,
        );
      ...
      debug &&
        core.debug(
          `Candidates for package ${pkg.name}: ${JSON.stringify(candidates, null, 2)}`,
        );
      ...
      debug &&
        core.info(
          `Filtered candidates for package ${pkg.name}: ${JSON.stringify(taggedCandidates, null, 2)}`,
        );
    }
