  ...
  async execute({
    ...
    for (const { package: pkg, versions } of packagesWithVersions) {
      ...
      if (toDeleteVersions.length > 0) {
        ...
        debug &&
          core.info(
            `Versions to delete for package ${pkg.name}: ${JSON.stringify(toDeleteVersions, null, 2)}`,
          );
      }
    }
