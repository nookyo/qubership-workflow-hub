    ...
    const packagePromises = packages.map(async (pkg) => {
      ...
      for (const v of pkg.versions) {
        for (const tag of v.metadata.container.tags) {
          ...
          if (
            low === "latest" ||
            excluded.some((pat) => this.wildcardMatcher.match(low, pat))
          ) {
      ...
      if (protectedTags.size > 0) {
        log.debug(
          ` [${pkg.name}] Protected tags: ${Array.from(protectedTags).join(", ")}`,
          MODULE,
        );
      }
      ...
      const protectedDigestPromises = Array.from(protectedTags).map(
        async (tag) => {
          try {
            ...
            log.warn(
              `Failed to fetch manifest for ${pkg.name}:${tag} — ${e.message}`,
            );
      ...
      log.debug(
        ` [${pkg.name}] After date & exclude filter: ${withoutExclude.length} versions`,
        MODULE,
      );
        ...
        withoutExclude.forEach((v) => {
          const tagsStr =
            v.metadata.container.tags.length > 0
              ...
              : "<no tags>";
          log.debug(
            `   - ${v.name.substring(0, 20)}... tags: [${tagsStr}]`,
            MODULE,
          );
        });
      }
      ...
      if (included.length > 0) {
        log.debug(
          ` [${pkg.name}] Include patterns: ${included.join(", ")}`,
          MODULE,
        );
      }
      ...
      if (taggedToDelete.length > 0) {
        ...
        log.debug(
          ` [${pkg.name}] taggedToDelete (${taggedToDelete.length}): ${preview}`,
          MODULE,
        );
      } else {
      ...
      const archLayers = withoutExclude.filter(
        (v) =>
          v.metadata.container.tags.length === 0 &&
      ...
      if (archLayers.length > 0) {
        ...
        log.debug(
          ` [${pkg.name}] archLayers (${archLayers.length}): ${preview}`,
          MODULE,
        );
      } else {
      ...
      if (debug) {
        if (danglingLayers.length > 0) {
          ...
          log.debug(
            `[${pkg.name}] danglingLayers (${danglingLayers.length}): ${preview}`,
            MODULE,
          );
        } else {
