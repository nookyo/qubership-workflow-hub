...
class ContainerStrategy extends AbstractPackageStrategy {
  ...
  async execute({
    ...
    for (let { package: pkg, versions } of packagesWithVersions) {
      ...
      for (const v of taggedCandidates) {
        for (const tag of v.metadata.container.tags) {
          try {
            ...
            debug &&
              core.debug(
                `Cannot get manifest digests for package ${pkg.name} with tag ${tag}: ${e.message}`,
              );
          }
        }
      }
      ...
      debug &&
        core.info(
          `All digests for package ${pkg.name}: ${Array.from(allDigests).join(", ")}`,
        );

      const layersToDelete = candidates.filter(
        (v) =>
      ...
      debug &&
        core.info(
          `Versions to delete for package ${pkg.name}: ${JSON.stringify(toDelete, null, 2)}`,
        );
    }
