...
class ContainerReport {
  async writeSummary(context) {
    ...
    core.summary.addRaw(
      `- Included Tags Patterns: ${includedTags.length ? includedTags.map((t) => `<code>${t}</code>`).join(", ") : "<code>None</code>"}\n`,
    );
    core.summary.addRaw(
      `- Excluded Tags Patterns: ${excludedTags.length ? excludedTags.map((t) => `<code>${t}</code>`).join(", ") : "<code>None</code>"}\n\n`,
    );
