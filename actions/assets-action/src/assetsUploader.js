...
class AssetUploader {
  ...
  async upload(assetPath) {
    ...
    if (!this.owner || !this.repo || !this.releaseTag) {
      throw new Error(
        `❗️ Incorrect initialization of AssetUploader: { owner: ${this.owner}, repo: ${this.repo}, releaseTag: ${this.releaseTag} }`,
      );
    }
    ...
    core.info(
      `Try Uploading asset: ${fileName} to release: ${this.releaseTag} in repo: ${repoArg}`,
    );
