...
async function run() {
  ...
  if (assignees.length === 0)
    return log.fail(`❗️ No assignees found in the configuration file.`);

  const author = pullRequest.user.login;
  log.dim(`Pull request author: ${author}`);

  const authorAssignee = assignees.filter(
  ...
  if (selfAssign && authorAssignee.length > 0) {
    ...
    if (count > assignees.length) {
      log.warn(
        `⚠️ Requested ${count} assignees, but only ${assignees.length} available. Using ${assignees.length}.`,
      );
  ...
  try {
    ...
    log.success(
      `✔️ Assigned ${assignees.length} user(s) to PR #${pullRequest.number}: ${assignees.join(", ")}`,
    );
  } catch (error) {
